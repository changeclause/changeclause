...
The MVP imports actual Vitest JSON reports through an explicit prepare/run/import protocol. Preparation binds source inventory, Git revision (Git mode), contract digest, runner name/version and time. Import requires the source to remain unchanged, validates report shape/counters, run start time, contained file paths and unique scenarios, and records artifact digest, status, capture time and each test's full name and own name. Git mode requires a clean worktree before and after; reports stay outside the analyzed project. A failed run cannot satisfy execution clauses. Retries explicitly reported in an artifact are inconclusive. Vitest 4 JSON does not expose all retry history: run with `--retry=0` for the pilot. The importer cannot prove that this option was used.
